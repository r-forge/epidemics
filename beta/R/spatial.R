...
### begin large while ###
    ...
    while(chooseAgain){
        ...
        if(type==4){
            ...
            if(!require(spdep, quiet=TRUE)) stop("spdep library is required.")
            n <- nrow(xy)
            ## find all-connected cn
            temp <- lapply(1:n, function(i) setdiff(1:n,i))
            class(temp) <- "nb"
            mstinput <- nb2listw(temp, nbdists(temp,xy), style="C")
            mst <- spdep::mstree(mstinput)
            cn <- split(as.integer(mst[,2]), factor(mst[,1], levels=1:n))
            class(cn) <- "nb"
        }
