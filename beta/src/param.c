...
void print_param(struct param *in){
	...
	for(i=0;i<in->npop;i++) printf("%d\t", in->popsizes[i]);
	printf("\ntransmission rate (beta): %.2f", in->beta);
	for(i=0;i<in->npop;i++) totpopsizes += in->popsizes[i];
	printf("\ntotal nb of susceptible: %d", totpopsizes);
